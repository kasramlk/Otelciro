...
function hotelNameOf(key){
  return HOTELS.find(h=>h.key===key)?.name || key;
}
...
function initHotelMultiSelect(){
  const root = el('hotelMultiSelect');
  if (!root) return;

  const trigger = root.querySelector('.multi-select__trigger');
  const textEl = root.querySelector('.multi-select__text');
  const dropdown = root.querySelector('.multi-select__dropdown');
  if (!trigger || !textEl || !dropdown) return;
  ...
  const updateTrigger = () => {
    const selected = HOTELS.filter(h => state.selectedHotels.has(h.key));
    const allSelected = selected.length === HOTELS.length;
    const label = allSelected
      ? 'Tüm Oteller'
      : (selected.length === 1 ? selected[0].name : `${selected[0].name} +${selected.length - 1}`);
    textEl.textContent = label;
    trigger.title = selected.map(s => s.name).join(', ');
  };

  const syncCheckboxes = () => {
    checkboxByKey.forEach((cb, key) => { cb.checked = state.selectedHotels.has(key); });
  };

  const commitSelection = () => {
    if (state.selectedHotels.size === 0){
      state.selectedHotels.add(HOTELS[0].key);
      syncCheckboxes();
    }
    updateTrigger();
    renderAll();
  ...
  HOTELS.forEach(h => {
    ...
    cb.type = 'checkbox';
    cb.value = h.key;
    cb.checked = state.selectedHotels.has(h.key);
    ...
    checkboxByKey.set(h.key, cb);

    cb.addEventListener('change', () => {
      if (cb.checked) state.selectedHotels.add(h.key);
      else state.selectedHotels.delete(h.key);
      commitSelection();
    });
  });

  const actions = document.createElement('div');
  actions.className = 'multi-select__actions';

  const btnAll = document.createElement('button');
  btnAll.type = 'button';
  btnAll.className = 'multi-select__action';
  btnAll.textContent = 'Tümü';

  const btnClear = document.createElement('button');
  btnClear.type = 'button';
  btnClear.className = 'multi-select__action';
  btnClear.textContent = 'Temizle';

  btnAll.addEventListener('click', () => {
    state.selectedHotels = new Set(HOTELS.map(h => h.key));
    syncCheckboxes();
    commitSelection();
  });

  btnClear.addEventListener('click', () => {
    state.selectedHotels = new Set([HOTELS[0].key]);
    syncCheckboxes();
    commitSelection();
  });
  ...
  const setOpen = (open) => {
    ...
    root.classList.toggle('multi-select--open', open);
  };
...
function renderKPIs(rows){
  ...
  const occWeighted = totalSold > 0
    ? rows.reduce((acc, r) => {
      const occ = (r.occ===null || r.occ===undefined) ? null : Number(r.occ);
      const sold = (r.soldNights===null || r.soldNights===undefined) ? null : Number(r.soldNights);
      if (!Number.isFinite(occ) || !Number.isFinite(sold)) return acc;
      return acc + (occ * sold);
    }, 0) / totalSold
    : average(rows.map(r=>r.occ));

  const adrWeighted = totalSold > 0 ? (totalRevenue / totalSold) : null;

  el('kpiRevenue').textContent = fmtEUR(totalRevenue);
  el('kpiOcc').textContent = occWeighted===null ? '—' : fmtPct(occWeighted);
  el('kpiSold').textContent = fmtNum(totalSold, 1);
  el('kpiAdr').textContent = adrWeighted===null ? '—' : fmtEUR2(adrWeighted);
}
...
function renderTable(){
  ...
  rows.sort((a,b)=>{
    switch(state.tableSort){
      case 'hotel': {
        const an = hotelNameOf(a.hotel);
        const bn = hotelNameOf(b.hotel);
      ...
      default: {
        const byMonth = monthIndex(a.month)-monthIndex(b.month);
        if (byMonth !== 0) return byMonth;
        return hotelNameOf(a.hotel).localeCompare(hotelNameOf(b.hotel), 'tr');
      }
    }
  });
...
function exportTableCsv(){
  ...
  rows
    ...
    .forEach(r => {
      const hotelName = hotelNameOf(r.hotel);
      lines.push([
